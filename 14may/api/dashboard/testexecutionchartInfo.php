...
while($cdsdata = mysqli_fetch_assoc($sqlCustomData)){
	...
	if($chart_type == "Pie"){
		...
		while($tcdata = mysqli_fetch_assoc($sqltestexecdata)){
			...
			$testresult = "";
			$testresultsqldata = mysqli_query($conn,"SELECT s_f_testresult from s_testcasefinal where testcaseId = '".$tcdata['s_t_id']."' and accountId = '".$accountId."' and s_f_testresult != ''  order by s_f_updatetime desc limit 1  ");
