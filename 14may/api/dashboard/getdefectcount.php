...
$defectsql = "SELECT
    ...
    IFNULL(SUM(defectstatusId IN (1, 3, 4, 5, 6, 7)),0) AS open_defects,
    IFNULL(SUM(defectstatusId IN (2, 8)),0) AS closed_defects,
    IFNULL(SUM(s_d_severity IN ('High')),0) AS critical_defects
	FROM s_defect where accountId = ? and s_d_createdtime <= ? ".$where." $aExtra";
