         ...
         success: function(data)
         {
          ...
          if(data !=null){
              if(data['id'] !=null){
                  ...
                  nArtifactschk = data['artifactschk']; 
...
/** bulk action to execute multiple test cases */
   $( document ).on( "click", ".bulkAction", function(e) {
      ...
      if(bulkcheckboxarr.length > 0){
            ...
            async: false,
            type: "POST", //The type which you want to use: GET/POST
            data: "type="+nType+"&&ids="+bulkcheckboxarr+"&&formtype=Testexecution&&executiontype="+nTestsuitetype+"&&testsuiteId="+paramval1+"&&schedchk="+nSchedulchk+"&&artifactschk="+nArtifactschk, //The variables which are going.
                  ...
                  if(data !=null){
                      if(data['id'] !=null){
                          ...
                          $("#schedularevery").attr("data-exist",data['schedularevery']);
                            ...
                            $("#artifactschk").prop("checked",false);
                            if(data['artifactschk'] == "1"){
                              $("#artifactschk").prop("checked",true);
                            }
...
$('#testsuitemodal').on('hidden.bs.modal', function () {
  ...
  $("#schedchk").prop("checked",false);
            $("#artifactschk").attr("checked",false);
  $("#schdulingsetting").addClass("hidden");
  $("#schdulingsettingchk").addClass("hidden");
  $(".autosetting").addClass("hidden");
...
//////////////////
$( document ).on( "change", "[id=artifactschk]", function(e) {
        // prevent default action
  e.preventDefault();
      // var schedulartype = $("#schedchk").val();

          $("#artifactschk_change").val(1);  
});
