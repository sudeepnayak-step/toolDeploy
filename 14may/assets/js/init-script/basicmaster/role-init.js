  ...
  var projTable =  $('#roleTbl')
    .on( 'error.dt', function ( e, settings, techNote, message ) {
        ...
        "ajax": {
            ...
            "type": 'POST',
            "data": function ( d ) {
            return {'formtype':'Role','enteredby':localStorage.getItem("id"),'accountId':localStorage.getItem("accountId"),'userempid':localStorage.getItem("userempid")}; // serializes the form's elements.
            }
        },
        "columnDefs": [
        	{
            ...
            }, 
            {
                ...
                'targets': [1],
                 "class": "text-left",
                "render": function ( data, type, row ) {
                  return '<a href="JavaScript:void(0)" id="editRole'+row[0]+'" data-id="'+row[0]+'" class="text-step" data-toggle="modal" data-target="#rolemodal" ><b>'+
                          data+
                         '</b> </a>';
                }
            }, 
            {
                'targets': [2],
                 "class": "text-left",
                "render": function ( data, type, row ) {
                  return '<a href="'+STEP_root+'master/rolemanagement.php?id='+row[0]+'" id=roleManagement'+row[0]+'" data-id="'+row[0]+'" class="text-step"  ><b>'+
                          data+
                         '</b> </a>';
                }
            }, 
            {
	            ...
	            "class":"text-left",
	    		render: function ( data, type, row ) {
					return '<a href="JavaScript:void(0)" id="deleteRole'+data+'" data-id="'+data+'" >'+
...
projTable.buttons().container().appendTo('#exportButtonsContainer');
 ...
 $( document ).on( "click", "[id^=deleteRole]", function(e) {
                  ...
                  confirm: {
                      ...
                      action: function(){
                          $.ajax({
                             ...
                             dataType:'json',
                             data:  {"id":currentId,'formtype':'Role','enteredby':localStorage.getItem("id"),'accountId':localStorage.getItem("accountId"),'userempid':localStorage.getItem("userempid")}, // serializes the form's elements.
...
/** rdit role based on id */
$( document ).on( "click", "[id^=editRole]", function(e) {
    // prevent default action
    e.preventDefault();
...
$('form[id="roleform"]').validate({
  ...
  submitHandler: function(form) {
     ...
     $.ajax({
        url: STEP_root+"api/saveRole.php", //The url where the server req would we made.
        async: false,
    ...
    $('thead input[id="select_all"]').on('click', function(e){ 
      ...
      if(this.checked){
         $('#roleTbl tbody input[type="checkbox"]:not(:checked)').trigger('click');
      } else {
         $('#roleTbl tbody input[type="checkbox"]:checked').trigger('click');
      }
   ...
   $('#roleTbl tbody').on('change', 'input[type="checkbox"]', function(){ 
...
/** bulk action tol make record active or inactive */
    $( document ).on( "click", ".bulkAction", function(e) {
        ...
        e.preventDefault();
          ...
          if(bulkcheckboxarr.length > 0){
                ...
                async: false,
                type: "POST", //The type which you want to use: GET/POST
                data: "type="+nType+"&&ids="+bulkcheckboxarr+"&&formtype=Role", //The variables which are going.
...
$('#rolemodal').on('hidden.bs.modal', function () {
  ...
  $("#rolename").attr("data-id","0");
  $('em.state-error').css('display', 'none');
});
})(jQuery);
