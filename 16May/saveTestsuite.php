...
if(!empty($id) && $id !="0") {
	...
	// Use prepared statements for INSERT query
	$sql = "INSERT INTO s_testsuite (projectId, releaseId, activityId, s_ts_tempid, s_ts_testsuitenum, s_ts_name, s_ts_description, s_ts_enteredby, accountId, s_ts_assignto, s_ts_type, s_ts_machinid, s_ts_cmd, s_ts_schedevery, s_ts_schedtype, s_ts_schedstarttime, s_ts_schedendtime,s_ts_artifacts, s_ts_schedchk) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?, '')";
	$stmt = mysqli_prepare($conn, $sql);
	mysqli_stmt_bind_param($stmt, "iiiisssiisssssssss", $projectId, $releaseId, $activityId, $testsuiteNum, $testsuiteIdstr, $testsuitename, $testsuitedesc, $enteredby, $accountId, $assignto, $type, $machinId, $command, $schedularevery, $schedulartype, $schedularstart, $schedularend,$artifactschk);
