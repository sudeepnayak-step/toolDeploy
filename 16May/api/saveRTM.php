...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
	...
	if($updateRecords !="" || (isset($attachmentsLogs) && !empty($attachmentsLogs)) ){
		...
		// Insert new record
		$sql = "INSERT INTO s_rtm (s_rtm_reqnum, s_rtm_tempid, projectId, releaseId, s_rtm_status, s_rtm_description, s_rtm_author, s_rtm_reviewer, s_rtm_enteredby, accountId,s_rtm_assignto,s_rtm_summary,s_rtm_priority,s_rtm_comment,s_rtm_editmode) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?,?,?,?,?)";
		$stmt = mysqli_prepare($conn, $sql);
		if ($stmt) {
			mysqli_stmt_bind_param($stmt, 'siissssiiiissss', $rtmstr, $rtmId, $projectId, $releaseId, $rtmstatus, $description, $enteredby, $reviewer, $enteredby, $accountId,$assignto,$summary,$priority,$comment,$editmode);
