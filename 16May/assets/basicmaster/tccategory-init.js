...
(function ($) {
   ...
   /** intialize testcase category table */
  var typeTable =  $('#categoryTbl').DataTable({
        "ajax": {
            "url": STEP_root+"api/getallBasicmaster.php",
            "type": 'POST',
            "data": function ( d ) {
            return {'formtype':'TCCategory','enteredby':localStorage.getItem("id"),'accountId':localStorage.getItem("accountId"),'userempid':localStorage.getItem("userempid")};
            }
        },
        "columnDefs": [
        	{
            ...
            }, 
            {
                ...
                'targets': [1],
                 "class": "text-left",
                "render": function ( data, type, row ) {
                  return '<a href="JavaScript:void(0)" id="editTCCategory'+row[0]+'" data-id="'+row[0]+'" class="text-step" data-toggle="modal" data-target="#categorymodal" ><b>'+
                          data+
                         '</b> </a>';
                }
            }, 
            {
               ...
               "visible": false,
	            "class":"text-left",
	    		render: function ( data, type, row ) {
					return '<a href="JavaScript:void(0)" id="deleteTCCategory'+data+'" data-id="'+data+'" >'+
    ...
    /** delete testcase category by id */
    $( document ).on( "click", "[id^=deleteTCCategory]", function(e) {
                  ...
                  confirm: {
                      ...
                      action: function(){
                         $.ajax({
                             ...
                             dataType:'json',
                             data:  {"id":currentId,'formtype':'TCCategory','enteredby':localStorage.getItem("id"),'accountId':localStorage.getItem("accountId"),'userempid':localStorage.getItem("userempid")}, // serializes the form's elements.
...
/** edit testcase category by id */
    $( document ).on( "click", "[id^=editTCCategory]", function(e) {
        // prevent default action
        e.preventDefault();
        ...
        /** save test case category changes */
$('form[id="categoryform"]').validate({
    ...
    submitHandler: function(form) {
        ...
        $.ajax({
            url: STEP_root+"api/saveTCCategory.php", //The url where the server req would we made.
            async: false,
   ...
   $('#categorytbl tbody').on('change', 'input[type="checkbox"]', function(){ 
    ...
    $( document ).on( "click", ".bulkAction", function(e) {
        ...
        if(bulkcheckboxarr.length > 0){
                ...
                async: false,
                type: "POST", //The type which you want to use: GET/POST
                data: "type="+nType+"&&ids="+bulkcheckboxarr+"&&formtype=TCCategory", //The variables which are going.
