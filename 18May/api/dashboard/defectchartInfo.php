...
if($type == "TestcaseDensity"){
	...
	left join s_defecttypemaster dt on dt.s_dt_id = d.defecttypeId where d.accountId = ? and d.testcaseId is not null && d.testcaseId != '' && d.testcaseId !='0'  $where  ".
...
//echo $ageingsql;		
	...
	from s_rtm_testcase rt 
	left join s_defect d on d.s_d_id = rt.defectId 
	left join s_project p on p.s_p_id = d.projectId 
	left join s_release r on r.s_r_id = d.releaseId 
	left join s_testcase t on t.s_t_id = d.testcaseId 
	left join s_defectstatusmaster ds on ds.s_ds_id = d.defectstatusId 
	left join s_defecttypemaster dt on dt.s_dt_id = d.defecttypeId where d.accountId = ? and rt.defectId is not null && rt.defectId != '' && rt.defectId !='0'  $where  ".
	($xVal == "Not Specified" || $xVal == "0" || $xVal == "" ? " AND (d.s_d_module is null or d.s_d_module = '' )" : " and d.s_d_module = '".$xVal."' ");
//echo $ageingsql;		
	$stmt = mysqli_prepare($conn, $ageingsql);
	mysqli_stmt_bind_param($stmt, "s",$accountId);
